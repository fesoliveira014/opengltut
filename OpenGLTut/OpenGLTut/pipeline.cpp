...
void Pipeline::InitRotateTransform(Matrix4f& m) const
{
	...
	m = rz * ry * rx; // Remember how chain matrix multiplication matters on linear transformations
}
...
void Pipeline::InitPerpectiveProj(Matrix4f& m) const
{
	const float ar			= m_persProj.Width / m_persProj.Height;
	const float zNear		= m_persProj.zNear;
	const float zFar		= m_persProj.zFar;
	const float zRange		= zNear - zFar;
	const float tangHalfFOV = tanf(ToRadian(m_persProj.FOV / 2.0f));

	m.m[0][0] = 1.0f / (tangHalfFOV * ar); m.m[0][1] = 0.0f				 ; m.m[0][2] = 0.0f					   ; m.m[0][3] = 0.0f;
	m.m[1][0] = 0.0f					 ; m.m[1][1] = 1.0f / tangHalfFOV; m.m[1][2] = 0.0f					   ; m.m[1][3] = 0.0f;
...
{
	Matrix4f Scale, Rotate, Translation, PerspecProj;

	InitScaleTransform(Scale);
	InitRotateTransform(Rotate);
	InitTranslationTransform(Translation);
	InitPerpectiveProj(PerspecProj);

	m_transformation = PerspecProj * Translation * Rotate * Scale;
	//m_transformation = Translation * Rotate * Scale;
